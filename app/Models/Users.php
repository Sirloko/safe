...
class Users extends Model {
    ...
    public function Validator(){
        ...
        $this->runValidation(new EmailValidator($this, ['field'=> 'email', 'msg' => 'Please enter valid email.']));
        $this->runValidation(new MaxValidator($this, ['field'=> 'email', 'rule'=> 150, 'msg' => 'Email must be less than to 150 char.']));
        //$this->runValidation(new NumericValidator($this, ['field'=> 'password', 'msg' => 'Password is not numeric.']));
        $this->runValidation(new MinValidator($this, ['field'=> 'username', 'rule'=> 6, 'msg' => 'Username must get to six.']));
        $this->runValidation(new MaxValidator($this, ['field'=> 'username', 'rule'=> 150, 'msg' => 'Username must be less than to 150 char.']));
        $this->runValidation(new UniqueValidator($this, ['field'=> 'username', 'msg' => 'Username already exists']));
        $this->runValidation(new RequiredValidator($this, ['field'=> 'password', 'msg' => 'Password is required.']));
        $this->runValidation(new MinValidator($this, ['field'=> 'password', 'msg' => 'Password must get to six.']));
        if($this->isNew()){
    ...
    public function findUserByUsernameOrEmail($email){
        return $this->findFirst(['conditions'=>'email OR username = ?', 'bind'=>[$email]]);
        
    }
    
    public  function findAll($user_id){
        return $this->findFirst(['conditions'=>"id = ?", 'bind'=>[$user_id]]);
        
    }
    public  function findUsers($acl){
        
        $sql = "SELECT * FROM users WHERE acl != '$acl' ";
        $tr = $this->query($sql)->results();
        // //H::dnd($tr[0]->username);
        // //$tre = json_decode(json_encode($tr), True);
        // //H::dnd();
         return $tr;
        //return $this->findFirst(['conditions'=>"acl != ?", 'bind'=>[$acl]]);
